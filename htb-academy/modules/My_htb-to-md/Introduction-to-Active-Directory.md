...
Active `Directory` makes information easy to find and use for `administrators and users`. `AD` is highly scalable, supports millions of objects per dom`ain`, and `allows` the creation of additional `domains` as an `organization` grows.
